-- Simple Solo Schema (No Teams)
-- Use this migration if you DON'T need team collaboration features
-- This is simpler and faster to set up for single-user SaaS apps
...
-- Function to get user tier limits (solo version - no team limits)
...
-- Trigger to create profile when user signs up (solo version)
CREATE OR REPLACE FUNCTION handle_new_user_solo()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER on_auth_user_created
  ...
  FOR EACH ROW EXECUTE FUNCTION handle_new_user_solo();
