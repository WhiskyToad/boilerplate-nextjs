...
export function StepsSidebar({
  ...
  const getStepIcon = (step: Step) => {
    const type = (step as any).step_type || step.type || (step as any).action_type || (step as any).event_type;

    if (!type) return <FaQuestion className="text-gray-400" />;
  ...
  const getStepTitle = (step: Step, index: number): string => {
    // Debug log to see what we're getting
    if (index === 0) {
      console.log('[StepsSidebar] Sample step data:', step);
    }
    ...
    // Generate descriptive title from element data
    const elementData = step.element || (step as any).element_data;
    const type = (step as any).step_type || step.type || (step as any).action_type || (step as any).event_type;

    // If no type, return generic step title
    if (!type) {
      console.warn('[StepsSidebar] No type found for step:', step);
      return `Step ${index + 1}`;
    }
    ...
    if (elementData?.textContent) {
      ...
      elementName = text.length > 30 ? `"${text.substring(0, 30)}..."` : `"${text}"`;
    } else if (elementData?.placeholder) {
      elementName = `"${elementData.placeholder}"`;
    } else if (elementData?.id) {
      elementName = `#${elementData.id}`;
    } else if (elementData?.tagName) {
      elementName = `<${elementData.tagName.toLowerCase()}>`;
    } else {
      elementName = 'element';
    }
    ...
    switch (type) {
      ...
      case 'input':
      case 'change':
        const value = (step as any).value || elementData?.value;
        if (value) {
      ...
      case 'submit':
        return `Submit form`;
      case 'navigation':
        const url = elementData?.url || (step as any).url;
        if (url) {
          try {
              ...
              <button
                ...
                className={`
                  ...
                  ${isCurrent
                    ...
                    : 'bg-gray-50 dark:bg-gray-800/50 border-2 border-transparent hover:border-gray-300 dark:hover:border-gray-700'
                  }
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      {(() => {
                        const stepType = (step as any).step_type || step.type || (step as any).action_type || (step as any).event_type;
                        return stepType ? (
                          <span className={`
                      ...
                      {(step.type === 'input' || step.type === 'change') && (step as any).value && (
